...
string get_fix_object(Aws::S3::S3Client *client, string input_bucket,
                      string handle) {
...
int upper_bound(vector<string> keys, string key) {
...
void do_bptree_get(string input_bucket, string tree_root, string minio_url,
                   string key, string output_bucket, string output_file) {
  ...
  while (true) {
    ...
    while (offset < keys.length() - 1) {
      uint64_t entry_size;
      memcpy(&entry_size, keys.data() + offset, sizeof(uint64_t));
      offset += sizeof(uint64_t);

      string entry;
      entry.resize(entry_size);
      memcpy(entry.data(), keys.data() + offset, entry_size);
      offset += entry_size;

      real_keys.push_back(entry);
    }

    auto idx = upper_bound(real_keys, key);
...
int main(int argc, char *argv[]) {
  ...
  Aws::InitAPI(options);
  {
    do_bptree_get(input_bucket, tree_root, minio_url, key, output_bucket,
                  output_file);
  }
